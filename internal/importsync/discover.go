...
// DiscoveryProvider keeps recovery reads independent from mutation capabilities.
type DiscoveryProvider interface {
	Name() compute.ProviderName
	List(context.Context, compute.ListServersQuery) ([]compute.ServerRecord, compute.Diagnostics)
}
...
func Discover(ctx context.Context, provider DiscoveryProvider, account compute.Account, filter DiscoverFilter) ([]Candidate, error) {
	if provider == nil {
	...
	for _, record := range records {
		candidate, ok, err := candidateFromRecord(provider.Name(), record, filter, reg)
		if err != nil {
			return nil, err
		}
		if !ok {
...
func candidateFromRecord(providerName compute.ProviderName, record compute.ServerRecord, filter DiscoverFilter, reg state.Registry) (Candidate, bool, error) {
	if record.Provider == "" {
	...
	localState, err := localStateStatus(reg, namespace, server)
	if err != nil {
...
func localStateStatus(reg state.Registry, namespace, server string) (string, error) {
	if namespace == "" || server == "" {
		return "missing", nil
	}
	...
	exists, err := state.Exists(defaultServerStatePath(namespace, server))
	if err != nil {
		return "", fmt.Errorf("check local state for %s/%s: %w", namespace, server, err)
	}
	if exists {
		return "partial", nil
	}
