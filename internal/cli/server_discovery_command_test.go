...
func TestServerStatusDiscoveryRejectsUnsupportedRegisteredState(t *testing.T) {
	createDiscoveryFixture(t, []string{"demoapp"})
	body := []byte(`{"schema_version":2,"namespace":"demoapp","server":"webapp"}`)
	if err := os.WriteFile(config.ServerStatePath("demoapp", "webapp"), body, 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "unsupported state schema version") {
		t.Fatalf("expected state schema error, got %v", err)
	}
}
...
func TestServerStatusFallsBackToNumberedPrompt(t *testing.T) {
	createDiscoveryFixture(t, []string{"demoapp", "sampleapp"})
	var out bytes.Buffer
	a := &app{stdin: strings.NewReader("2\n"), stdout: &out, stderr: io.Discard, providers: readProviderRegistry(t), selectServerMatch: func([]string) (string, bool, error) {
		return "", false, nil
	}}
	cmd := a.serverStatusCmd()
	cmd.SetOut(&out)
	...
	if !strings.Contains(out.String(), `"namespace": "sampleapp"`) || !strings.Contains(out.String(), "select server") {
		t.Fatalf("numbered fallback not used:\n%s", out.String())
	}
}
...
func createDiscoveryFixture(t *testing.T, namespaces []string) {
	...
	for _, namespace := range namespaces {
		...
		if err := credentials.Save(cfg, credentials.Set{Project: namespace, Server: "webapp", ServerProvider: "acct", Tailscale: "ts"}); err != nil {
			t.Fatal(err)
		}
		st := state.State{Namespace: namespace, Project: namespace, Server: "webapp", Compute: state.ComputeState{Provider: "hetzner", Namespace: namespace, Server: "webapp", ID: "42", Name: namespace + "-webapp", Location: "fsn1", Size: "cpx22", Image: "ubuntu-24.04"}, Tailscale: state.TailscaleState{Name: namespace + "-webapp"}}
		stPath := config.ServerStatePath(namespace, "webapp")
		if err := state.Save(stPath, st); err != nil {
			t.Fatal(err)
		}
		reg.Upsert(state.RegistryEntry{Project: namespace, Server: "webapp", StatePath: stPath})
	}
...
func createDiscoveryCredentialFixture(t *testing.T) {
	...
	for _, namespace := range []string{"demoapp", "sampleapp"} {
		cfg := config.ExampleServer(namespace, "webapp")
		if err := credentials.Save(cfg, credentials.Set{Project: namespace, Server: "webapp", ServerProvider: namespace + "-token", Tailscale: "ts"}); err != nil {
			t.Fatal(err)
		}
		st := state.State{Namespace: namespace, Project: namespace, Server: "webapp", Compute: state.ComputeState{Provider: "hetzner", Namespace: namespace, Server: "webapp", ID: "42", Name: namespace + "-webapp", Location: "fsn1", Size: "cpx22", Image: "ubuntu-24.04"}, Tailscale: state.TailscaleState{Name: namespace + "-webapp"}}
		stPath := config.ServerStatePath(namespace, "webapp")
