...
func TestNamespaceCreateWaitsForNamespaceOperation(t *testing.T) {
	createTestHome(t)
	...
	go func() {
		done <- (&app{stdout: io.Discard}).runNamespaceCreate(context.Background(), "demoapp")
	}()
	select {
	case err := <-done:
		unlock()
		t.Fatalf("namespace create bypassed namespace operation lock: %v", err)
	case <-time.After(serverOperationLockProbe):
	}
	if _, err := os.Stat(config.NamespaceConfigDir("demoapp")); !os.IsNotExist(err) {
		unlock()
		t.Fatalf("namespace create mutated files while blocked: %v", err)
	}
	unlock()
	select {
	case err := <-done:
	...
	case <-time.After(time.Second):
		t.Fatal("namespace create did not resume after lock release")
	}
}
