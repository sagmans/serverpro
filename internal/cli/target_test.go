...
func TestValidateStateTargetRejectsResourceNameMismatches(t *testing.T) {
	...
	serverNameErr := validateStateTarget(cfg, state.State{Namespace: "prod", Server: "web", Compute: state.ComputeState{Name: "prod-api"}})
	...
	tunnelNameErr := validateStateTarget(cfg, state.State{Namespace: "prod", Server: "web", Cloudflare: state.CloudflareState{Name: "prod-api"}})
