...
func (a *app) resolveSudoPasswordWithLabel(cfg config.Config, label string) (string, error) {
	envName, err := sudoPasswordEnvName(cfg.Namespace, cfg.Server)
	if err != nil {
		return "", err
	}
	key := cfg.Namespace + "\x00" + cfg.Server
...
func sudoPasswordEnvName(namespace, server string) (string, error) {
	if namespace == "" {
	...
	return encodeEnvPart(namespace) + "_" + encodeEnvPart(server) + "_SUDOPASS", nil
}
