...
func (a *app) loadConfigFromRegistryTarget() (config.Config, string, bool, error) {
	...
	cfg, err := loadManagedServerConfig(entry.ConfigPath, configTarget{Namespace: entry.Namespace, Server: targetServer(entry.Server), ResourceNames: entry.ResourceNames})
	if err != nil {
		return cfg, "", true, err
	}
