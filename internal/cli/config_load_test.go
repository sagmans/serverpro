...
func TestLoadConfigOnlyAppliesTargetOverrides(t *testing.T) {
	...
	if loaded.Namespace != "prod" || loaded.Server != "api" {
		t.Fatalf("target = %s/%s", loaded.Namespace, loaded.Server)
	}
...
func TestApplyConfigTargetOverridesRefreshesNamesForProjectOnlyOverride(t *testing.T) {
	...
	if cfg.Namespace != "new" || cfg.Server != "web" {
		t.Fatalf("target = %s/%s", cfg.Namespace, cfg.Server)
	}
...
func TestLoadConfigOnlyUsesRegistryTarget(t *testing.T) {
	...
	if loaded.Namespace != "prod" || loaded.Server != "web" {
		t.Fatalf("target = %s/%s", loaded.Namespace, loaded.Server)
	}
