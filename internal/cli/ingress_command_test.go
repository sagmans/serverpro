...
func (p blockingStatusProvider) Status(_ context.Context, ref compute.ServerRef) (compute.ServerStatus, compute.Diagnostics) {
	close(p.entered)
	<-p.release
	return compute.ServerStatus{Record: ref.Record, Power: "running", PublicIPv4: "203.0.113.99"}, nil
}

func TestConcurrentStatusAndIngressUpdatesBothSurvive(t *testing.T) {
	createServerReadFixture(t)
	provider := blockingStatusProvider{entered: make(chan struct{}), release: make(chan struct{})}
	statusApp := &app{stdout: io.Discard, stderr: io.Discard, project: "demoapp", provider: "hetzner", providers: testRegistryWithProvider(t, provider)}
	status := statusApp.serverStatusCmd()
	status.SetOut(io.Discard)
	status.SetErr(io.Discard)
	status.SetArgs([]string{"webapp"})
	statusDone := make(chan error, 1)
	go func() { statusDone <- status.Execute() }()
	<-provider.entered

	ingressApp := &app{stdout: io.Discard, stderr: io.Discard, project: "demoapp"}
	add := ingressApp.ingressCmd()
	...
	st, err := state.Load(config.Expand(config.ServerStatePath("demoapp", "webapp")))
	if err != nil {
		t.Fatal(err)
	}
	if st.Compute.PublicIPv4 != "203.0.113.99" || len(st.Ingress) != 1 || st.Ingress[0].Hostname != "app.example.com" {
		t.Fatalf("concurrent updates lost: %+v", st)
	}
}
