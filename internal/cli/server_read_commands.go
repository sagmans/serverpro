...
func (a *app) resolveServerSSHUser(st state.State, cfgPath string) (string, error) {
	if cfgPath == "" {
		cfgPath = config.ServerConfigPath(st.Namespace, st.Server)
	}
...
func (a *app) persistAdminUsername(cfgPath string, st state.State, user string) error {
	mutate := func(cfg *config.Config) error {
		...
		if cfg.Namespace == "" {
			cfg.Namespace = st.NamespaceName()
		}
		if cfg.Server == "" {
	...
	if err := config.Update(cfgPath, mutate); err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			return err
		}
		// Missing config is an explicit recovery case; conditional publication
		// cannot overwrite a create that appears during recovery.
		cfg := config.ExampleServer(st.Namespace, st.Server)
		if err := mutate(&cfg); err != nil {
			return err
		}
		if err := config.SaveIfUnchanged(cfgPath, cfg, nil, false); err != nil {
			if errors.Is(err, config.ErrSourceChanged) {
				return fmt.Errorf("config changed while persisting admin username; retry")
			}
			return err
		}
	}
...
func (a *app) localServerRows() ([]serverReadRow, error) {
...
func (a *app) refreshServerRow(ctx context.Context, stPath string, st state.State) (serverReadRow, error) {
	...
	if status.Record.ID != "" && (st.Compute.PublicIPv4 != status.PublicIPv4 || st.Compute.PublicIPv6 != status.PublicIPv6) {
		...
		if err := state.Update(config.Expand(stPath), func(current *state.State) error {
			current.Compute.PublicIPv4 = status.PublicIPv4
			current.Compute.PublicIPv6 = status.PublicIPv6
			updated = *current
			return nil
		}); err != nil {
			return serverReadRow{}, err
...
func (a *app) loadServerReadRef(name string) (serverReadRef, error) {
	...
	stPath := a.statePath
	cfgPath := ""
	if reg, err := state.LoadRegistry(config.RegistryPath()); err == nil {
...
func (a *app) serverStateMatches(name string) ([]serverStateMatch, error) {
	...
	for _, entry := range reg.List(a.project) {
		...
		st, err := state.Load(config.Expand(entry.StatePath))
		if err != nil || !a.matchesServerState(st) {
			continue
		}
...
func serverMatchOptions(matches []serverStateMatch) []string {
	...
	for _, match := range matches {
		st := match.State
		options = append(options, fmt.Sprintf("%s provider=%s", st.Namespace, st.Compute.Provider))
	}
	return options
}
...
func (a *app) matchesServerState(st state.State) bool {
	if a.project != "" && st.Namespace != a.project {
		return false
	}
...
func serverRowFromState(st state.State) serverReadRow {
	return serverReadRow{Namespace: st.Namespace, Server: st.Server, Provider: st.Compute.Provider, Location: st.Compute.Location, Size: st.Compute.Size, Image: st.Compute.Image, PublicIPv4: st.Compute.PublicIPv4, Tailscale: tailscaleSummary(st), SSH: sshSummary(st), Ingress: ingressSummary(st)}
}

func serverRecordFromState(st state.State) compute.ServerRecord {
	return compute.ServerRecord{Provider: compute.ProviderName(st.Compute.Provider), Namespace: st.Namespace, Server: st.Server, ID: st.Compute.ID, Name: st.Compute.Name, Location: st.Compute.Location, Size: st.Compute.Size, Image: st.Compute.Image, PublicIPv4: st.Compute.PublicIPv4, PublicIPv6: st.Compute.PublicIPv6, ManagedResources: append([]compute.ManagedResourceRef(nil), st.Compute.ManagedResources...), ProviderState: st.Compute.ProviderState}
}
