...
func (a *app) runCreateCommand(cmd *cobra.Command) error {
	...
	if a.dryRun {
		...
		return lifecycle.BuildPlan(cfg).Write(a.stdout, a.jsonOut)
	}
	cfg, stPath, err := a.prepareConfig(a.project, a.server)
	if err != nil {
		return err
	}
...
func (a *app) computeAccountForConfig(cfg config.Config, creds credentials.Set) (compute.Account, error) {
	...
	return compute.Account{Name: cfg.Project + "/" + cfg.Server, Provider: provider.Name(), Token: creds.ServerProvider, Scope: cfg.Project + "/" + cfg.Server}, nil
}
...
func (a *app) loadCreatePreviewConfig() (config.Config, error) {
	if path := a.initialConfigPath(a.project, a.server); path != "" && fileExists(config.Expand(path)) {
		...
		return cfg, a.validateCreatePreviewTarget(cfg)
	}
	if a.configPath != "" {
...
func explicitCreatePreviewConfig(project, server string) config.Config {
...
func (a *app) validateCreatePreviewTarget(cfg config.Config) error {
	check := cfg
	...
	return validateCreateImageReference(provider, check.Compute.Image)
}
