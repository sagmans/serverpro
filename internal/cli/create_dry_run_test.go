...
func TestServerCreateFlagsCoverPromptedDetails(t *testing.T) {
	...
	cmd.SetArgs([]string{"server", "create", "web", "-n", "mynamespace", "-p", "hetzner", "--dry-run", "--non-interactive", "--compute-name", "mynamespace-web", "--location", "nbg1", "--size", "cpx31", "--image", "debian-12", "--admin-user", "ops", "--tailscale-tailnet", "example.ts.net", "--tailscale-tags", "tag:serverpro-mynamespace", "--ingress", "none", "--egress-mode", "open"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out.String(), "size=cpx31 image=debian-12 location=nbg1") || !strings.Contains(out.String(), "open best-effort policy") {
...
func TestCreatePreviewRejectsMissingCatalogSelection(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.edit(&cfg)
			err := validateCreatePreviewTarget(cfg)
