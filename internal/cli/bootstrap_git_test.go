...
func TestBootstrapGitRunsDeployAccessByDefaultWhenInteractive(t *testing.T) {
	...
	for _, want := range []string{"Set up Git SSH deploy access for a private GitHub repo? [Y/n]", "read-only deploy key", `"status": "complete"`, `"target": "git"`} {
...
func TestMaybeSetupGitDeployAccessRejectsInvalidRepoBeforeGeneratingKey(t *testing.T) {
	...
	err := a.maybeSetupGitDeployAccess(context.Background(), config.Config{}, state.State{}, "")
