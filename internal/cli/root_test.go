...
func TestRootHelpShowsResourceFirstSurface(t *testing.T) {
	...
	for _, want := range []string{"namespace", "server", "provider", "catalog", "ingress", "doctor"} {
		listing := "\n  " + want
...
func TestParentCommandsRejectUnknownSubcommands(t *testing.T) {
	for _, parent := range []string{"namespace", "server", "provider", "catalog", "ingress"} {
...
func TestUnsupportedScopedFlagsAreRejected(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "yes flag with catalog locations", args: []string{"-y", "catalog", "locations"}, want: `--yes is not supported by "serverpro catalog locations"`},
