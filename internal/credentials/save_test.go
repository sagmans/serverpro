...
func TestSaveWritesPrivateJSON(t *testing.T) {
	...
	if creds.Project != "prod" || creds.Namespace != "prod" || creds.Server != "server" || creds.ServerProvider != "h" {
		t.Fatalf("saved namespace = %q project = %q server = %q server_provider=%q", creds.Namespace, creds.Project, creds.Server, creds.ServerProvider)
	}
