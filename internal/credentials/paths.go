...
func validateConfigScope(cfg config.Config) error {
	...
	if !config.CredentialsPathScopedToServer(cfg.Credentials.JSONPath, cfg.Namespace, cfg.Server) {
		return fmt.Errorf("credentials.json_path must be scoped to server %q in namespace %q", cfg.Server, cfg.Namespace)
	}
	return nil
}
...
func safeCredentialPath(namespace, server, path, action string) (string, error) {
	if !config.CredentialsPathScopedToServer(path, namespace, server) {
		return "", fmt.Errorf("credentials.json_path must be scoped to server %q in namespace %q", server, namespace)
	}
