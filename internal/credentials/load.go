...
func LoadPartial(cfg config.Config) (Set, error) {
	...
	if path == "" {
		return Set{Namespace: cfg.Project, Project: cfg.Project, Server: cfg.Server}, nil
	}
	path, err := safeCredentialPath(cfg.Project, cfg.Server, path, "load")
	if err != nil {
		return Set{}, err
	}
	if !fileExists(path) {
	...
	return creds, creds.ValidateTarget(cfg.Project, cfg.Server)
}
