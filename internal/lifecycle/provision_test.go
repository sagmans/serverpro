...
func TestRunEnsuresTailscalePolicyBeforeAuthKeyAndTracksManagedPolicy(t *testing.T) {
	...
	if got := strings.Join(ts.calls, ","); !strings.HasPrefix(got, "snapshot-devices,ensure-policy,create-key") {
		t.Fatalf("tailscale calls = %s", got)
	}
	if !st.Tailscale.PolicySSHRule || strings.Join(st.Tailscale.PolicyTagOwners, ",") != "tag:serverpro-prod" || strings.Join(st.Tailscale.PolicySSHTags, ",") != "tag:serverpro-prod" {
