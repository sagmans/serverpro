...
func TestUpsertFindListAndRemoveRegistryEntry(t *testing.T) {
	...
	reg.Upsert(RegistryEntry{Project: "prod", Server: "api", StatePath: "/state/prod/api.json"})
	reg.Upsert(RegistryEntry{Project: "other", Server: "web", StatePath: "/state/other/web.json"})
...
func TestUpsertPreservesCreatedAtForExplicitDefaultServer(t *testing.T) {
	...
	reg := NewRegistry()
	reg.Upsert(RegistryEntry{Project: "prod", Server: "default", StatePath: "/state/prod/default.json", CreatedAt: created})
	reg.Upsert(RegistryEntry{Project: "prod", Server: "default", StatePath: "/state/prod/new-default.json"})
...
func TestRemoveProjectDeletesAllProjectEntries(t *testing.T) {
	reg := NewRegistry()
	...
	if got := reg.List(""); len(got) != 1 || got[0].Project != "other" {
