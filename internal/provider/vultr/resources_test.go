...
func TestCreateInstanceSendsIPv4OnlyAndBase64UserData(t *testing.T) {
	...
	if got["region"] != "ewr" || got["plan"] != "vc2-1c-2gb" || got["os_id"] != float64(1743) {
...
func TestListFirewallRulesPagesAllRules(t *testing.T) {
	calls := 0
	handlerErr := testhttp.NewHandlerErrorRecorder(t)
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if calls == 1 {
			if r.URL.Query().Get("cursor") != "" {
				handlerErr.Record(w, "unexpected first-page cursor %q", r.URL.Query().Get("cursor"))
				return
			}
			_, _ = w.Write([]byte(`{"firewall_rules":[{"id":1,"action":"accept","ip_type":"v4","protocol":"udp","port":"41641","subnet":"0.0.0.0","subnet_size":0}],"meta":{"links":{"next":"cursor-2","prev":""}}}`))
			return
		}
		if r.URL.Query().Get("cursor") != "cursor-2" {
			handlerErr.Record(w, "unexpected next-page cursor %q", r.URL.Query().Get("cursor"))
			return
		}
		_, _ = w.Write([]byte(`{"firewall_rules":[{"id":2,"action":"accept","ip_type":"v6","protocol":"udp","port":"41641","subnet":"::","subnet_size":0}],"meta":{"links":{"next":"","prev":"cursor-1"}}}`))
	}))
	defer ts.Close()

	rules, err := NewWithHTTP("token", ts.URL, ts.Client()).ListFirewallRules(context.Background(), "fw-9")
	handlerErr.Check()
	if err != nil {
		t.Fatal(err)
	}
	if calls != 2 || len(rules) != 2 || rules[0].ID != 1 || rules[1].ID != 2 {
		t.Fatalf("calls=%d rules=%+v", calls, rules)
	}
}
