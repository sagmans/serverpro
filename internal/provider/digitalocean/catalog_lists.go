...
func (c Client) Regions(ctx context.Context) ([]Region, error) {
	return pagedList(func(page int) ([]Region, *int, error) {
		...
		err := c.api.Do(ctx, http.MethodGet, catalogListPath("/regions", page, nil), nil, &res)
		return res.Regions, nextPage(res.pageMeta.Links.Pages.Next), err
	})
}
...
func (c Client) Sizes(ctx context.Context) ([]Size, error) {
	return pagedList(func(page int) ([]Size, *int, error) {
		...
		err := c.api.Do(ctx, http.MethodGet, catalogListPath("/sizes", page, nil), nil, &res)
		return res.Sizes, nextPage(res.pageMeta.Links.Pages.Next), err
	})
}
...
func (c Client) Images(ctx context.Context) ([]Image, error) {
	return pagedList(func(page int) ([]Image, *int, error) {
		...
		err := c.api.Do(ctx, http.MethodGet, catalogListPath("/images", page, map[string]string{"type": "distribution"}), nil, &res)
		return res.Images, nextPage(res.pageMeta.Links.Pages.Next), err
	})
}
