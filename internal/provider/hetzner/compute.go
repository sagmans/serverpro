...
func (p ComputeProvider) Create(ctx context.Context, request compute.CreateServerRequest) (compute.ServerRecord, compute.Diagnostics) {
	...
	if err != nil {
		...
		return record, p.failure(request.Account.Token, "provider access policy create failed", err)
	}
...
func (p ComputeProvider) List(ctx context.Context, query compute.ListServersQuery) ([]compute.ServerRecord, compute.Diagnostics) {
	...
	records, err := recoverServerRecords(servers, firewalls)
	if err != nil {
		return nil, p.failure(query.Account.Token, "provider recovery inventory invalid", err)
	}
	return records, compute.Diagnostics{{Status: compute.Pass, Message: "provider server list loaded"}}
}
...
func ensureAccessPolicy(ctx context.Context, client Client, request compute.CreateServerRequest) (int64, error) {
	...
	if request.CheckpointProviderState != nil {
		if err := request.CheckpointProviderState(map[string]string{"access_policy_id": strconv.FormatInt(firewall.ID, 10)}); err != nil {
			return firewall.ID, err
		}
	}
...
func partialServerRecordFromCreateRequest(request compute.CreateServerRequest, accessPolicyID int64) compute.ServerRecord {
	return compute.ServerRecord{
		...
		ProviderState: map[string]string{
			"access_policy_id": strconv.FormatInt(accessPolicyID, 10),
		},
	}
}
...
func mapCatalog(catalog Catalog, location string) compute.Catalog {
	...
	for _, image := range catalog.Images {
		...
		mapped := compute.Image{Name: image.Name, Description: image.Description, Architecture: image.Architecture, OSFlavor: image.OSFlavor, OSVersion: image.OSVersion}
