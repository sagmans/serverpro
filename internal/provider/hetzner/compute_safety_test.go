...
func TestComputeProviderDeleteRejectsInvalidLiveOwnership(t *testing.T) {
	tests := []struct {
		name       string
		serverJSON string
		wantError  string
	}{
		{name: "unlabeled", serverJSON: `{"server":{"id":42,"name":"prod-web"}}`, wantError: "ownership"},
		{name: "mismatched", serverJSON: `{"server":{"id":42,"name":"other-web","labels":{"serverpro.namespace":"other","serverpro.server":"web"}}}`, wantError: "ownership mismatch"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				switch {
				case r.Method == http.MethodGet && r.URL.Path == "/servers/42":
					_, _ = w.Write([]byte(test.serverJSON))
				case r.Method == http.MethodDelete && r.URL.Path == "/servers/42":
					deleteCalled = true
				default:
					handlerErr.Record(w, "unexpected request %s %s", r.Method, r.URL.Path)
			...
			if diagnostics.Passed() || diagnostics.Err() == nil || !strings.Contains(diagnostics.Err().Error(), test.wantError) || deleteCalled {
				t.Fatalf("diagnostics=%v deleteCalled=%t", diagnostics.Err(), deleteCalled)
			}
		})
	}
...
func TestComputeProviderCreateReconcilesServerByNameOnCreateError(t *testing.T) {
	...
	if diagnostics.Passed() || record.ID != "42" || !ok || policyID != "9" || len(record.ProviderState) != 0 {
