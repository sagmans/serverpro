...
type CreateServerRequest struct {
	...
	ManagedResources []ManagedResourceRef `json:"managed_resources,omitempty"`
...
// FailureDiagnostic keeps the public message safe while retaining machine-readable cause identity.
func FailureDiagnostic(message string, cause error) Diagnostic {
	diagnostic := Diagnostic{Status: Fail, Message: message}
	if cause != nil {
		diagnostic.cause = diagnosticError{message: message, cause: cause}
	}
	return diagnostic
}
...
func (d Diagnostics) Err() error {
	...
	for _, diagnostic := range d {
		...
		if diagnostic.cause != nil {
			messages = append(messages, diagnostic.cause)
			continue
		}
