...
func TestFailureDiagnosticPreservesCauseWithoutSerializingIt(t *testing.T) {
	cause := errors.New("private provider detail")
	diagnostics := Diagnostics{FailureDiagnostic("provider request failed", cause)}
	payload, err := json.Marshal(diagnostics)
	if err != nil {
		t.Fatal(err)
	}
	if !errors.Is(diagnostics.Err(), cause) || strings.Contains(string(payload), cause.Error()) || !strings.Contains(string(payload), "provider request failed") {
		t.Fatalf("diagnostics=%+v err=%v payload=%s", diagnostics, diagnostics.Err(), payload)
	}
}
