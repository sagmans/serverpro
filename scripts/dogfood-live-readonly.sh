...
run_readonly_dogfood() {
	...
	for provider in "${providers[@]}"; do
		...
		run_live_ok catalog "live catalog locations $provider" "$bin" --non-interactive -p "$provider" catalog locations
		run_live_ok catalog "live catalog sizes $provider" "$bin" --non-interactive -p "$provider" catalog sizes --location "$location"
		run_live_ok catalog "live catalog images $provider" "$bin" --non-interactive -p "$provider" catalog images --location "$location"
