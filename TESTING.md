...
| `internal/bootstraptools` | Embedded host-tool bootstrap script and target selection. | Canonical mise specification-to-manifest/doctor mapping, focused Git prerequisite sequencing and gh-only repair, executable configuration calls, shared readiness/final-verification probes, generated-script target/package apply-and-upgrade behavior, refreshed apt freshness check, exact uv/Rust/gh/rg/fd/ast-grep/sem/inspect pins, release-shaped uv/ast-grep output, absent/present/failure legacy `sg` config migration, GitHub release checksums and architecture gates, inspect digest-before-execution, explicit backend and Rust-profile validation, forced same-version Rust/checksum-tool repair, component-scoped repair, malformed/missing/wrong-version probes, Rust default-component checks, script env, shell syntax, no `curl\|sh`, and executable exact-primary-key GPG allow/deny cases. |
...
| `internal/doctor` | Local/provider/remote validation report. | Local tool checks, one-call compute/mesh/explicit Cloudflare tunnel snapshot fakes, injected public-SSH probes, exact Git identity/signing reads and fixes with first-failure preservation, explicit batched read plans with conditional reads, strict baseline replay, no fixes after per-command output overflow, planned fix/recheck delegation including package-refresh authority, sequential fallback, remote-only sudo retry, Tailscale node, bootstrap tools, Cloudflare connector. |
...
| Current deep-review remediation | 85.1% |
