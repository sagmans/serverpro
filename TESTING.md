...
| `serverpro server delete` | Destructive preview, confirmation, authority-drift rejection, read-only external preflight, ownership validation, compute delete, partial-failure recovery evidence, Tailscale cleanup, proven-created Cloudflare cleanup, retained adopted/imported/legacy tunnels, local state removal. | `internal/cli/server_power_delete_command_test.go`, `internal/cli/server_delete_cleanup*_test.go`, `internal/cli/server_delete_preflight_test.go`, provider delete tests; invalid external credentials stop before compute mutation, post-compute cleanup failure returns redacted partial JSON and retains authority, and pre-lock registry replacement cannot redirect or consume the approved state target | dry-run fixture cases plus `internal/e2e/delete_failure_test.go` compiled invalid-preflight and post-compute retry journeys, and production-orchestrated full-chain device cleanup evidence | Live dogfood cleanup path |
...
| Delete and cleanup | typed managed-resource/migration/coalescing/conflict and tunnel-provenance units | all-provider typed access-policy checkpoints and preview, pre-lock registry/state-path binding, state/registry/config/credential authority revalidation, read-only external preflight before compute mutation, redacted partial JSON with retained retry authority, DigitalOcean all-resource preflight plus bounded legacy-selector recovery, legacy retry compatibility, same-server workflow serialization, fake provider and server-scoped cleanup; adopted/imported/legacy tunnels retained; no global ACL mutation | production-orchestrated full-chain device cleanup, compiled invalid-preflight and post-compute retry journeys, plus dry-run fixtures | Live cleanup after create |
